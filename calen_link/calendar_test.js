/*
   ...
   -----------------------------------------------------
*/
//[✓]indexs.phpへanswer1.phpの挿入
//[ ]日付を表示している変数または処理を見つける。出力：linkgo関数→処理挿入場所に処理を挿入 showTtl関数
//[ ]$image変数の処理結果を配列に挿入する。比較用関数159行目
//[ ]$image変数をJavaScriptにおいて使用可にする。
//[ ]カレンダーに日付を表示する際に$imageと日付を表示するJS変数を比較する。
//[ ]変数同士を比較した後、写真が入っている日付を操作可能にする。
...
function dateReset(){
	//現在の日付を取得
	nowDate = new Date();
	theYear = nowDate.getFullYear();	//年を4桁の整数で取得する
	theMonth = nowDate.getMonth();	//月を0から11の整数で取得する
